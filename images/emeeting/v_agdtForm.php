...
if(isset($edit) AND $edit){ // Edit
	$action = "agenda/agdtEdit";
} else { // Add
	$action = "agenda/agdtAdd";
}
			...
			<td>
				...
				if(isset($file) && $file->num_rows > 0){
					foreach($file->result() as $row_file){
						echo '<div class="MultiFile-label"><a href="javascript:void(0);" class="delete" id="'.$row_file->agfl_id.'"><img alt="" src="'.base_url().'images/emeeting/del.png"></a><span class="MultiFile-title" title="File selected: delete.png"><a href="'.base_url().'uploads/emeeting/docs/'.$row_file->agfl_nname.'" target="_blank" >'.$row_file->agfl_oname.'</a></span></div>';
					}
				}
				?>
				</div>
			...
			if($flag_parent_id != -1){
				...
				<tr>
					<?php
						...
						$getAdd = $p_agdt_id."/".$agdt_level."/".$mt_id."/".$cms_id."/".$ck_edit."/".$no_parent;
					?>
					<td colspan="2" align="right">
