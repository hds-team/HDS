...
function pntCharge(pnt_id){
	$.fancybox({
		...
		'onClosed'			: function() {
								...
								?>";
								sendPost("pntCharge",{"step":"3","mt_id":<?php echo $mt_id; ?>,"cms_id":<?php echo $cms_id; ?>,"pnt_id":pnt_id},url,"","");
							}
	});
}
...
function pntEdit(pnt_id){
	...
	?>";
	sendPost("pntEdit",{"step":"3","mt_id":<?php echo $mt_id; ?>,"cms_id":<?php echo $cms_id; ?>,"pnt_id":pnt_id,"flag_edit":"1"},url,"","");
}
...
function pntDel(pnt_id, warnMsg){
	...
	?>";
	sendPost("pntDel",{"step":"3","mt_id":<?php echo $mt_id; ?>,"cms_id":<?php echo $cms_id; ?>,"pnt_id":pnt_id,"flag_del":"1"},url,"","คุณต้องการลบ \""+warnMsg+"\" ใช่หรือไม่");
}
			...
			if(isset($flag) && $flag == "continue"){
				...
				echo form_open($this->config->item("emt_path").$action,"frmNext");
					...
					<div align="center">
						...
						<?php
						if( $status != "edit" ){
							echo "<input type=\"submit\" class=\"da-button blue\" name=\"btnnextto4\" value=\"ถัดไป  >\"  style=\"width:150px;\" />";
							} else {
