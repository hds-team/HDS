...
<div id="content">
	...
	<div class="grid_4_center">
		...
		<h3>กำหนดกลุ่มผู้เข้าร่วมประชุม</h3>
		 </span>
		 </div>
		<?php
			$row_gp = ($qu_gp) ? $qu_gp->row() : null;
			
			$action = "groupMeeting/updateGroup";
		...
		<div style="padding:25px;">
			<b>ชื่อกลุ่ม : </b>
			<input type="text" name="gp_name" size="60" value="<?php echo $row_gp->gp_name; ?>" class="validate" title="ชื่อกลุ่ม" />&nbsp;<span class="red">*</span>
		</div>
	 <div class="grid_3_center"  >
			...
			if(isset($dArray) && !empty($dArray)){
				...
				foreach($dArray as $d_key => $d_val){
					...
					echo "<p style=\"padding-left:".$tab."px;\"><b>".$d_val["deptName"]."</b><br/>";
					...
					foreach($rs_ps->result() as $row_ps){
						$check = '';
						foreach($rs_ps_gp as $row_gpt){
							if($row_ps->personId == $row_gpt->gpt_personId){
								$check = "checked";
								break;
							}
						}
						echo "<li><input ".$check." type=\"checkbox\" name=\"person[]\" value=\"".$row_ps->personId."\" style=\"margin:5px 10px 5px 0px;\" /><span>".$row_ps->fName." ".$row_ps->lName."</span></li>";		
					}
					echo "</ul>";
					echo "</p>";
				}
		...
		<div align="center" style="padding:25px;">
			<input type="hidden" name="gp_id" value=<?php echo $row_gp->gp_id;?> />
