...
function acStart(obj){
	if(get == "get_researcher"){			//-- ดึงชื่อนักวิจัย สำหรับเพิ่มคณะผู้วิจัย
		...
		ep_rs_id = obj.parent().find("input[name=ep_rs_id]").val();
		if(ep_rs_id > 0){
			obj.parents("div:first").find("input[name=ep_rs_id]").val("");
			//obj.parents("div:first").find("input[name=ep_rs_name]").val("");
		}
	}
	else if(get == "get_project2ig"){		//-- ดึงชื่อโครงการวิจัย ในเมนูบันทึกการบูรณาการงานวิจัย
