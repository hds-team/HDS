    ...
    <table width="95%" border="0" cellspacing="0" cellpadding="0" summary="" bgcolor="#FFFFFF" align="center">
		...
		<tr><td>&nbsp;</td></tr>
                    <tr>
                        ...
                        <td valign="top" align="left" style="font-size: 16pt; font-weight: bold; color: #898989;">{SYSTEMNAME}</td>
                    ...
                    <tr><td valign="top" align="left" style="font-size: 14pt; font-weight: normal; color: #11387D;">{SYSTEMSGRP}</td></tr>
                </table>
            </td>
        </tr>
    ...
    function retsetTdAlign ( )
    {
        var refn = document.getElementById("bodyspace");
        if (refn)
            resetAlignNow(refn);
    }
    function resetAlignNow (el)
    {
        if (el && el.childNodes) {
            for (var i = 0; i < el.childNodes.length; i++) {
                var node = el.childNodes[i];
                if (node.tagName && node.tagName == "TD" && !node.align)
                    node.align = "left";
                resetAlignNow(node);
            }
        }
